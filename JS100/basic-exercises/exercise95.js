console.clear;
/* exercise 95 Is it true?
We are experimenting with some code to get more comfortable working with objects. Run the snippet below and explain why "It's true!" is never output.
*/ 

let obj = {
  num: 42,
  'property name': 'string value',
  true: false,
  fun: function() {
    console.log('Harr Harr!');
  },
};

for (let prop in obj) {
  if (prop === true) {  //evaluates against boolean true in this stmt
    console.log("It's true!");
  }
}

//All keys within an object are 'String' types, Therefore prop === true never is true.

/* official solution
The condition of our if statement on line 11 returns false for all properties, 
because property names are always strings, while the body of our for loop looks 
for the Boolean value true.
*/