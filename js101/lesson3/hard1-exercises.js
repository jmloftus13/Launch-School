...
/*These functions do not return the same thing. The function first() returns the expected value of "hi there", 
but second() returns undefined without throwing any errors.

The reason for this odd behavior is that semicolons, in JavaScript, are technically optional. However, 
when you omit them, the JavaScript engine inserts them where it thinks you need them. In second, 
it inserts a semicolon after the word return, so the function returns undefined. 
...
However, most developers consider it poor practice since you're relying on the engine 
to make decisions for you, and those decisions may not be what you intended.*/
...
let numArray = object["first"]; //wasn't clear on this as a ref to first: [1]
numArray.push(2);
...
//using pass by reference it modifies the object
let one = ["one"]; 
let two = ["two"];
let three = ["three"];
...
/* ls solution
There are several ways to fix this code. We'll look at one possible solution to both 
...
but the other error remains: it doesn't return false when encountering an invalid 
component such as 257 or abc. Ben used a break statement to break out of the while loop, 
but that causes control to fall through to the return true statement. 
